...
tau_scan = np.geomspace(0.1e-12, 2e-12, 15) #[V/m] Amplitudes de pompe à simuler pour le scan en fluence, couvrant une plage large pour observer la saturation
...
amplitude = 4.40e7 #[V/m] amplitude de la pompe utilisée dans le code
R = 20e-6  #[m] Rayon du faisceau
...
# ==============================================================================
# PARTIE D : VARIATION DU TEMPS DE RECOMBINAISON
# ==============================================================================
print("\n" + "="*50)
print(" PARTIE D : VARIATION DU TEMPS DE RECOMBINAISON")
print("="*50)
...
print("Lancement du scan en temps de recombinaison...")

for i, tau_val in enumerate(tau_scan):
    current_pump = amplitude * base_pulse
    ...
    amp_probe_curr = amplitude * 0.005 
    ...
    print(f"   Step {i+1}/{len(tau_scan)} : Tau={tau_val:.2e}", end="\r")

print("\nScan terminé.")

# --- Plot Saturation ---
fig2, ax2 = plt.subplots(figsize=(3.37, 3.37), constrained_layout=True)

x_axis = np.array(tau_scan)
y_axis = np.array(R_sat_curr_list) * 100 # en %

ax2.plot(x_axis*1e12, y_axis, 'r-o', linewidth=2, markersize=4)
ax2.set_xlabel(r'Recombination time (ps)')
...
plt.savefig(f'{results_folder}figure/tau_simulation.png', dpi=600)
print(f"Figure sauvegardée sous : {results_folder}figure/tau_simulation.png")
...
results_df = pd.DataFrame({
    'tau': x_axis*1e12, # Convertir en ps
    'R':y_axis,
})

csv_filename = f'{results_folder}data/tau_simulation_{amplitude:.2e}.csv'
